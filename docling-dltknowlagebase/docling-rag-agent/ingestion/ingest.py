...
class DocumentIngestionPipeline:
    ...
    def __init__(
        ...
        self.ceramic_client = CeramicClient()
    ...
    async def _ingest_single_document(self, file_path: str) -> IngestionResult:
        ...
        # Save to PostgreSQL (this will also create Ceramic streams)
    ...
    async def _save_to_postgres(
        ...
        """Save document and chunks to PostgreSQL and create Ceramic streams."""
        async with db_pool.acquire() as conn:
            async with conn.transaction():
                ...
                # Prepare Ceramic stream data for batch creation
                ceramic_chunks_data = []
                chunk_ids = []
                
                # Insert chunks and collect data for Ceramic
                for chunk in chunks:
                    ...
                    # Prepare data for Ceramic stream
                    embeddings_metadata = {
                        "model": os.getenv("EMBEDDING_MODEL", "text-embedding-3-small"),
                        "dimension": len(chunk.embedding) if hasattr(chunk, 'embedding') and chunk.embedding else 1536
                    }
                    
                    ceramic_chunks_data.append({
                        "doc_id": document_id,
                        "chunk_id": chunk_id,
                ...
                # Create Ceramic streams in batch (outside transaction to avoid blocking)
                # This happens after chunk insertion to ensure chunk_ids exist
                
        # Batch create Ceramic streams (async operation, doesn't block)
        try:
            ceramic_results = await self.ceramic_client.batch_create_streams(ceramic_chunks_data)
            
            # Save Ceramic stream mappings to database
            async with db_pool.acquire() as conn:
                async with conn.transaction():
                    for i, result in enumerate(ceramic_results):
                        if result:
                            chunk_id = chunk_ids[i]
                            await conn.execute(
                                """
                                INSERT INTO ceramic_streams 
                                (chunk_id, stream_id, latest_cid, signer_did, ipfs_pins, stream_metadata)
                                ...
                                result['commit_cid'],
                                result['signer_did'],
                                json.dumps(result.get('ipfs_pins', [])),
                                json.dumps({})
                            )
                            logger.info(f"Saved Ceramic stream mapping for chunk {chunk_id}")
                        else:
                            logger.warning(f"No Ceramic stream created for chunk {chunk_ids[i]}")
            
            logger.info(f"Created {len([r for r in ceramic_results if r])} Ceramic streams for document {document_id}")
        except Exception as e:
            logger.error(f"Failed to create Ceramic streams: {e}")
            # Continue anyway - Ceramic is optional enhancement
