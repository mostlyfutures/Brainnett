...
def main():
    ...
    if not os.getenv("DATABASE_URL"):
        ...
        print(f"{Colors.YELLOW}📊 To configure your database:{Colors.END}")
        print(f"   1. In VS Code: Open Command Palette (Cmd/Ctrl+Shift+P)")
        print(f"   2. Type: 'Tasks: Run Task'")
        print(f"   3. Select: 'Configure Database'")
        print(f"   4. Or run: python3 setup_database.py")
        print()
        sys.exit(1)

    if not os.getenv("OPENAI_API_KEY"):
        print(f"{Colors.RED}✗ OPENAI_API_KEY environment variable is required{Colors.END}")
        print(f"{Colors.YELLOW}📝 To configure your API key:{Colors.END}")
        print(f"   1. In VS Code: Open Command Palette (Cmd/Ctrl+Shift+P)")
        print(f"   2. Type: 'Tasks: Run Task'")
        print(f"   3. Select: 'Configure OpenAI API Key'")
        print(f"   4. Or run: python setup_config.py")
        print()
        sys.exit(1)
